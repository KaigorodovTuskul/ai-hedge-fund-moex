...
def analyze_fisher_growth_quality(financial_line_items: list) -> dict:
    ...
    revenues = [v for fi in financial_line_items if (v := getattr(fi, 'revenue', None)) is not None]
    if len(revenues) >= 2:
    ...
    eps_values = [v for fi in financial_line_items if (v := getattr(fi, 'earnings_per_share', None)) is not None]
    if len(eps_values) >= 2:
    ...
    rnd_values = [v for fi in financial_line_items if (v := getattr(fi, 'research_and_development', None)) is not None]
...
def analyze_margins_stability(financial_line_items: list) -> dict:
    ...
    op_margins = [v for fi in financial_line_items if (v := getattr(fi, 'operating_margin', None)) is not None]
    if len(op_margins) >= 2:
    ...
    gm_values = [v for fi in financial_line_items if (v := getattr(fi, 'gross_margin', None)) is not None]
    if gm_values:
...
def analyze_management_efficiency_leverage(financial_line_items: list) -> dict:
    ...
    ni_values = [v for fi in financial_line_items if (v := getattr(fi, 'net_income', None)) is not None]
    eq_values = [v for fi in financial_line_items if (v := getattr(fi, 'shareholders_equity', None)) is not None]
    ...
    # 2. Debt-to-Equity
    debt_values = [v for fi in financial_line_items if (v := getattr(fi, 'total_debt', None)) is not None]
    ...
    # 3. FCF Consistency
    fcf_values = [v for fi in financial_line_items if (v := getattr(fi, 'free_cash_flow', None)) is not None]
...
def analyze_fisher_valuation(financial_line_items: list, market_cap: float | None) -> dict:
    ...
    # Gather needed data
    net_incomes = [v for fi in financial_line_items if (v := getattr(fi, 'net_income', None)) is not None]
    fcf_values = [v for fi in financial_line_items if (v := getattr(fi, 'free_cash_flow', None)) is not None]
