...
def analyze_risk_reward(financial_line_items: list, prices: list) -> dict:
    ...
    equity_values = [v for fi in financial_line_items if (v := getattr(fi, 'shareholders_equity', None)) is not None]
...
def analyze_druckenmiller_valuation(financial_line_items: list, market_cap: float | None) -> dict:
    ...
    ebit_values = [v for fi in financial_line_items if (v := getattr(fi, 'ebit', None)) is not None]
    ebitda_values = [v for fi in financial_line_items if (v := getattr(fi, 'ebitda', None)) is not None]

    # For EV calculation, let's get the most recent total_debt & cash
    debt_values = [v for fi in financial_line_items if (v := getattr(fi, 'total_debt', None)) is not None]
    cash_values = [v for fi in financial_line_items if (v := getattr(fi, 'cash_and_equivalents', None)) is not None]
